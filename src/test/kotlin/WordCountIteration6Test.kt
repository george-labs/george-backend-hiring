...
class WordCountIteration6Test {
    ...
    @Test
    fun `given text input when collectWordCountElements, then and totalCount and uniqueCount then collect total, uniques and average un-hyphened`() {
        ...
        val averageValue ="%.2f".format( counter.takeAverage(result,total)).toDouble()


        assertEquals(3, total)
        assertEquals(3, uniques)
        assertEquals(4.67, averageValue)
    }


    @Test
    fun `given text input when collectWordCountElements, then and totalCount and uniqueCount then collect total, uniques and average Hyphened`() {
        val stopFile = tempFileCreation(listOf("had", "a"))
        val counter: WordCountInterface = WordCountService(stopFile)
        ...
        assertEquals(6.43, averageValue)
    }
